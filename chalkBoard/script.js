 ...
 */
  ...
  function drawWithChalk(context, fromX, fromY, toX, toY, colorIdx) {
    ...
    var length = Math.round(
      Math.sqrt(Math.pow(toX - fromX, 2) + Math.pow(toY - fromY, 2)) / (5 / brushDiameter),
    );
  ...
  function redrawGrid(centerX, centerY, diameter) {
    ...
    for (
      var x = fromX + distance * Math.ceil((centerX - diameter - fromX) / distance);
    ...
    var fromY =
      ...
      Math.floor((drawingCanvas[1].height - distance) / 2 / distance) * distance;
    for (
      var y = fromY + distance * Math.ceil((centerY - diameter - fromY) / distance);
    ...
    switch (message.content.type) {
      ...
      case 'init':
        ...
        for (var id = 0; id < 2; id++) {
          ...
          drawingCanvas[id].yOffset =
            (drawingCanvas[id].height - storage[id].height * drawingCanvas[id].scale) / 2;
        }
        clearCanvas(0);
        clearCanvas(1);
        if (!playback) {
