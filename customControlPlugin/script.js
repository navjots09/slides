...
const initCustomControls = function (Reveal) {
  ...
  if (Reveal.isSpeakerNotes() && new URLSearchParams(location.search).get('controls') == 'false')
    return;
  ...
  toggleButton.innerHTML =
    '<span id="collapse-customcontrols">' +
    collapseIcon +
    '</span>' +
    '<span id="expand-customcontrols">' +
    expandIcon +
    '</span>';
  ...
  for (var i = 0; i < config.controls.length; i++) {
    ...
    control.innerHTML =
      '<button ' +
      (config.controls[i].title ? 'title="' + config.controls[i].title + '" ' : '') +
      'onclick="' +
